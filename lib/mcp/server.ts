...
export function createReviewCardsMcpServer(userId: string) {
  ...
  server.registerTool(
    ...
    async ({ section_id, items }) =>
      safeCall(async () => {
        const { inserted } = await addLanguageItems(userId, section_id, items);
        return `${inserted} item(ns) adicionado(s) à seção de idioma.`;
      })
  );
  ...
  server.registerTool(
    ...
    async ({ section_id }) =>
      safeCall(async () => {
        const items = await listLanguageItems(userId, section_id);
